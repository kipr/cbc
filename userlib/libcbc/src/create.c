...
int create_wall() {
  ...
  char *bptr = buffer;
	...
	create_write_byte(3);  // send 3 packets
	create_write_byte(8);  // virtual wall seen or not (1 byte)
	create_write_byte(13); // virtual wall or home base seen or not (1 byte)
	create_write_byte(27); // signal strength (2 bytes)
	CREATE_WAITFORBUFFER(buffer,4,-1)
	gc_wall=(*(bptr++));
	gc_wall_hb=(*(bptr++));
	gc_wall_amt=256*(*(bptr++));
	gc_wall_amt=gc_wall_amt + (*(bptr++));
	CREATE_FREE;
	return(0);
}
...
int create_angle() {
  ...
  char *bptr = buffer;
	...
	CREATE_WAITFORBUFFER(buffer,2,-1)
	  newangle = 256*(*(bptr++));
	newangle= newangle + (*(bptr++));
	CREATE_FREE;
...
int create_distance() {
  ...
  char *bptr = buffer;
	...
	CREATE_WAITFORBUFFER(buffer,2,-1)
	  incDistance=256*(*(bptr++));
	incDistance= incDistance + (*(bptr++));
...
int create_velocity() {
  ...
  char *bptr = buffer;
	CREATE_BUSY;
	create_write_byte(149);
	create_write_byte(4);  // send 4 packets
	create_write_byte(39); 
	create_write_byte(40);
	create_write_byte(41);
	create_write_byte(42);
	CREATE_WAITFORBUFFER(buffer,8,-1)
	  gc_vel=256*(*(bptr++));
	gc_vel=gc_vel  + (*(bptr++));     // curent create_drive velocity in mm/sec (-500 to +500)
	if(gc_vel > 32767) gc_vel = 65536 - gc_vel;
	gc_radius=256*(*(bptr++));
	gc_radius=gc_radius + (*(bptr++));  // current create_drive radius in mm
	if(gc_radius > 32767) gc_radius = 65536 - gc_radius;
	gc_rvel=256*(*(bptr++));
	gc_rvel=gc_rvel + (*(bptr++));    // current drive_direct right wheel velocity in mm/sec
	if(gc_rvel > 32767) gc_rvel = 65536 - gc_rvel;
	gc_lvel=256*(*(bptr++));
	gc_lvel=gc_lvel + (*(bptr++));    // current drive_direct left wheel velocity in mm/sec
...
int create_cargo_bay_inputs()
{
  ...
  char *bptr = buffer;
	...
	create_write_byte(33);
	CREATE_WAITFORBUFFER(buffer,3,-12)
	gc_digital_in=(*(bptr++));//pins 15,6,18,5,17
	gc_analog_in=(*(bptr++))*256;
	gc_analog_in=gc_analog_in+(*(bptr++));//analog input is on pin 4
	CREATE_FREE;
	return(0);
}
